...
export function PWAUpdateSettings({ onBeforeUpdate }: Props) {
  ...
  const [latestVersion, setLatestVersion] = useState<PwaVersionInfo | null>(null);
    ...
    <div className="pt-4 border-t border-white/[0.06]">
      ...
      <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-4 space-y-3">
        <div className="flex items-center justify-between gap-3 text-sm">
          <span className="text-white/45">Installed release</span>
          <code className="text-xs text-white/70">
            {formatRelease(installedRelease)}
          </code>
        </div>
        <div className="flex items-center justify-between gap-3 text-sm">
        ...
        {latestVersion && (
          <>
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="text-white/45">Latest release</span>
              <code className="text-xs text-white/70">
                v{latestVersion.release} ({shortCommit(latestVersion.commit)})
              </code>
            </div>
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="text-white/45">Latest build</span>
              <code className="text-xs text-white/70">
                {shortVersion(latestVersion.version)}
              </code>
            </div>
          </>
        )}
